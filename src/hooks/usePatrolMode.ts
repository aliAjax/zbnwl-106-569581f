...
import type { Plot, MaintenanceRules, DailyTask } from '../types/plot';
...
export const usePatrolMode = ({ plots, rules, getDailyTasks, updatePlot }: UsePatrolModeProps) => {
  ...
  const hasUnfinishedProgress = useCallback((): boolean => {
    try {
      ...
      if (saved) {
        const parsed: PatrolProgress = JSON.parse(saved);
        if (parsed.date === today) {
          const plotsList = patrolPlots();
          return parsed.currentIndex < plotsList.length;
        }
      }
    } catch {
      // ignore
  ...
  const getProgressStats = useCallback(() => {
    const plotsList = patrolPlots();
    return {
      total: plotsList.length,
      completed: progress.completedPlotIds.length,
      current: progress.currentIndex + 1,
      remaining: Math.max(0, plotsList.length - progress.currentIndex),
    };
  }, [patrolPlots, progress.completedPlotIds.length, progress.currentIndex]);
